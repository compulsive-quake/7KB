...
- The right activity rail (`RightActivityBar.tsx`) has two groups separated by a `flex-1` spacer (WebStorm tool-window stripe layout): a top group of right-docked side panels — Run (`useRuns`) and Action Log (`useActionLog`) — and a bottom group of full-width bottom-docked tool windows — Terminal (`useTerminal`). Each panel has its own Zustand store with `panelOpen`/`setPanelOpen`/`togglePanel`.
- Right-dock mutual exclusion: Run and Action Log share the right dock, so opening one closes the other (enforced at the rail click AND in `RunPanel`'s `panelOpen` effect because `startRun` opens it without going through the rail). The Terminal is on a separate bottom dock and is independent — it can be open alongside a right-docked panel; do NOT couple it into the right-dock exclusion.
